defmodule RemoteIEx.State do
  @moduledoc false
  # This state is exchanged between IEx.Server and
  # IEx.Evaluator which is why it is a struct.
  defstruct cache: '', counter: 1, prefix: "iex", gets: nil, puts: nil
  @type t :: %__MODULE__{}
end

defmodule RemoteIEx.Server do
  @moduledoc false
  ...
  defp run(opts) when is_list(opts) do
    puts = Keyword.get(opts, :puts, &IO.puts/2)
    puts.(:stdio,
  ...
  def start_evaluator(opts) do
    evaluator =
      opts[:evaluator] ||
        :proc_lib.start(RemoteIEx.Evaluator, :init, [:ack, self(), Process.group_leader(), opts])
  ...
  defp loop(state, evaluator, evaluator_ref) do
    ...
    input = spawn(fn -> io_get(self_pid, gets, prefix, counter) end)
  ...
  defp wait_input(state, evaluator, evaluator_ref, input) do
    receive do
      ...
      {:input, ^input, {:error, :interrupted}} ->
        io_error(state.puts, "** (EXIT) interrupted")
  ...
  defp handle_take_over(
    ...
    try do
      ...
      type, detail ->
        io_error(state.puts, "** (IEx.Error) #{type} when printing EXIT message: #{inspect(detail)}")
    end
  ...
  defp iex_state(opts) do
    ...
    gets = Keyword.get(opts, :gets, &IO.gets/2)
    puts = Keyword.get(opts, :puts, &IO.puts/2)
    %RemoteIEx.State{prefix: prefix, gets: gets, puts: puts}
  end

  ## IO

  defp io_get(pid, gets, prefix, counter) do
    prompt = prompt(prefix, counter)
    send(pid, {:input, self(), gets.(:stdio, prompt)})
  end
  ...
  defp io_error(puts, result) do
    puts.(:stdio, IEx.color(:eval_error, result))
  end
