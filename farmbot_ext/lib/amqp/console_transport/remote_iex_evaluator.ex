defmodule RemoteIEx.Evaluator do
  @moduledoc false
  ...
  defp loop_state(server, history, opts) do
    ...
    state = %{
      gets: opts[:gets] || &IO.gets/2,
      puts: opts[:puts] || &IO.puts/2,
      binding: binding,
      scope: scope,
      env: env,
      server: server,
  ...
  defp eval_dot_iex(state, path) do
    try do
      ...
      kind, error ->
        io_result(state, "Error while evaluating: #{path}")
        print_error(state.puts, kind, error, __STACKTRACE__)
        System.halt(1)
    end
  end
  ...
  defp io_inspect(state, result) do
    io_result(state, inspect(result, IEx.inspect_opts()))
  end

  defp io_result(state, result) do
    state.puts.(:stdio, IEx.color(:eval_result, result))
  end

  ## Error handling

  defp print_error(puts, kind, reason, stacktrace) do
