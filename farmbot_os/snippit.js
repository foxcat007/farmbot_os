...
var html = `
<div class="widget-wrapper" id="remote_console">
   <div class="widget-header">
      <span class="pt-popover-wrapper">
         <div class="pt-popover-target"><i class="fa fa-gear"></i></div>
      </span>
      <button class="fb-button red e-stop gray">E-STOP</button>
      <h5>Console</h5>
      <i class="fa fa-question-circle help-icon">
         <div class="help-text"> Remote shell to FarmbotOS. </div>
      </i>
   </div>
   <div class="widget-body">
     ...
     <div class="block" id="gets_div">
       <label id="gets"> iex(1)> </label>
       <input style="width: 90%;" type="text" id="gets_input" />
     </div>
   </div>
</div>`.trim();
...
document.getElementById('remote_console').scrollIntoView()

var ansiUp = new AnsiUp;
current_bot.client.subscribe("bot/device_1/remote_console");
current_bot.client.on('message', function (topic, event) {
  if (topic === "bot/device_1/remote_console") {
    var message = JSON.parse(event);
    console.dir(message);

    switch(message.kind) {
      case "puts": {
        var html = ansiUp.ansi_to_html(message.args.message);
      ...
      case "gets": {
        var text = ansiUp.ansi_to_text(message.args.message);
...
input.addEventListener("keyup", function(event) {
  if (tabKeyPressed) {
    ...
    data = JSON.stringify({kind: "gets_response", args: {message: input.value}});
    cmdHistory.push(input.value);
    cmdHistoryIndex = 0;
    input.value = "";
    current_bot.client.publish("bot/device_1/remote_console", data);
